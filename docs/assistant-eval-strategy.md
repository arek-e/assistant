...
Start with the Bun-powered eval runner in `packages/evals/src/run.ts` that tests primitive functions directly. The first version does not require a live model call for retrieval, lifecycle, or routing assertions. The runner uses the Worker-exported SQLite-backed Canonical Memory Store through a Bun SQLite adapter so fixtures exercise SQL and FTS behavior outside Cloudflare.
