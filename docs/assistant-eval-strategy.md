...
Start with the Bun-powered eval runner in `evals/run.ts` that tests primitive functions directly. The first version does not require a live model call for retrieval, lifecycle, or routing assertions.
