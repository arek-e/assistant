...
```bash
bun create cloudflare@latest . --category remote-template --template cloudflare/agents-starter --no-deploy --no-git --accept-defaults
bun install
bun dev
```
...
Workers AI runs remotely during development, so run `bunx wrangler login` before
`bun dev` if Wrangler is not already authenticated.
...
- **Effect** — `effect` is installed and used in `src/effects.ts` for typed server-side tool logic
